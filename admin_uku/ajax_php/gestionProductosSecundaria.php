...
{
	if($_POST['tipo'] == 'formCrearProductosSecundaria'){
		...
		if(validarLogueado($_conection)){
			...
			if ($error == 0) {
				...
				if ($rs_sql) {
					...
					//Devolvemos Id
					$result['id'] = $id;
					$result['link'] = 'inicio.php?page='.$variables['nombreSeccion3'].'&'.$prefijoAnt.'id='.$id_ant.'&id='.$id;
				}else{
					//Error sql
					$result["isOk"] = false;
	...
	if($_POST['tipo'] == 'formEditarProductosSecundaria'){
