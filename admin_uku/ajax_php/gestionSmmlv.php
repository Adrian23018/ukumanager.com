...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') 
{
	...
	if($_POST['tipo'] == 'formTraerDatos') {
		...
		if(validarLogueado($_conection)){
			...
			$result[$nombreCampos."-nombre"] = utf8_encode($row_sql[$prefijo.'salario']);
	...
	if($_POST['tipo'] == 'formEditarSmmlv'){
		...
		if(validarLogueado($_conection)){
			...
			$filtered = filter_var($dataNombre, FILTER_VALIDATE_INT);
			if( $filtered === false || $dataNombre <= 0 ) {
