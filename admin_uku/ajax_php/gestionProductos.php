...
{
	if($_POST['tipo'] == 'formCrearProductos'){
		...
		if(validarLogueado($_conection)){
			...
			/*$sql = sprintf("SELECT * FROM ".$tabla." WHERE ".$prefijo."nombre=%s",
			...
			if ($error == 0) {
				...
				if ($rs_sql) {
					...
					//Devolvemos Id
					$result['id'] = $id;
					$result['link'] = 'inicio.php?page='.$variables['nombreSeccion3'].'&'.$prefijoAntAnt.'id='.$id_antant.'&'.$prefijoAnt.'id='.$id_ant.'&id='.$id;
				}else{
					//Error sql
					$result["isOk"] = false;
	...
	if($_POST['tipo'] == 'formEditarProductos'){
		...
		if(validarLogueado($_conection)){
			...
			//verificar que usuario no se encuentre en la base de  datos
			/*$sql = sprintf("SELECT * FROM ".$tabla." WHERE ".$prefijo."nombre=%s AND ".$prefijo."id!=%s",
				GetSQLValueString($dataNombre ,"text"),
				GetSQLValueString($id ,"int")
			);
			$rs_sql = mysqli_query($_conection->connect(), $sql);
