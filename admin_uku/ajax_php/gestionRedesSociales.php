...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') 
{
	if($_POST['tipo'] == 'formTraerProgress') {
		...
		if(validarLogueado($_conection)){
			...
			if(isset($progressPestanasArray['general'])){
				$totalGeneral = 0;
				if ($row_sql[$prefijo."facebook"])
					$totalGeneral++;		
				if ($row_sql[$prefijo."twitter"])
					$totalGeneral++;
				if ($row_sql[$prefijo."google_plus"])
					$totalGeneral++;
				if ($row_sql[$prefijo."instagram"])
					$totalGeneral++;
				if ($row_sql[$prefijo."youtube"])
					$totalGeneral++;
				if ($row_sql[$prefijo."linkedin"])
					$totalGeneral++;
	...
	if($_POST['tipo'] == 'formTraerDatos') {
		...
		if(validarLogueado($_conection)){
			...
			$result[$nombreCampos."-facebook"] = utf8_encode($row_sql[$prefijo.'facebook']);
			$result[$nombreCampos."-twitter"] = utf8_encode($row_sql[$prefijo.'twitter']);
			$result[$nombreCampos."-google-plus"] = utf8_encode($row_sql[$prefijo.'google_plus']);
			$result[$nombreCampos."-instagram"] = utf8_encode($row_sql[$prefijo.'instagram']);
			$result[$nombreCampos."-youtube"] = utf8_encode($row_sql[$prefijo.'youtube']);
			$result[$nombreCampos."-linkedin"] = utf8_encode($row_sql[$prefijo.'linkedin']);
	...
	if($_POST['tipo'] == 'formEditarRedesSociales'){
		...
		if(validarLogueado($_conection)){
			...
			$dataFacebook	= utf8_decode($data[$nombreCampos."-facebook"]);
			$dataTwitter	= utf8_decode($data[$nombreCampos."-twitter"]);
			$dataGooglePlus	= utf8_decode($data[$nombreCampos."-google-plus"]);
			$dataInstagram	= utf8_decode($data[$nombreCampos."-instagram"]);
			$dataYoutube	= utf8_decode($data[$nombreCampos."-youtube"]);
			$dataLinkedin	= utf8_decode($data[$nombreCampos."-linkedin"]);
			...
			if ($error == 0) {
				if (!$row_sql[$prefijo."id"]) {
					$sql = sprintf("	INSERT INTO ".$tabla." 
							...
							GetSQLValueString($id,"int"),
							GetSQLValueString($_SESSION[_sessionIdioma],"int"),
							GetSQLValueString($dataFacebook,"text"),
							GetSQLValueString($dataTwitter,"text"),
							GetSQLValueString($dataGooglePlus,"text"),
							GetSQLValueString($dataInstagram,"text"),
							GetSQLValueString($dataYoutube,"text"),
							GetSQLValueString($dataLinkedin,"text")
						);
				}else{
